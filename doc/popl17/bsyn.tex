...
$v := \plus(x, y)$ by setting $\theta(v)$ to the polynomial $(\theta(x) + \theta(y))\in {\cal A}$.
Once we have the substitution $\theta$ resulting
from symbolically executing the whole program, we can check if 
...
For example, $ab$ is not random, but $ax$ and $b+ax$ are random.
