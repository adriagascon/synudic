...
\def\gena{\mbox{${\mathtt{genx}}$}}
\def\genb{\mbox{${\mathtt{geny}}$}}
\def\genc{\mbox{${\mathtt{genr}}$}}
\def\gend{\mbox{${\mathtt{genu}}$}}
\def\inputa{${\mathtt{inputa}}$}
\def\inputb{${\mathtt{inputb}}$}
...
\begin{figure*}[tb]
  \begin{tabular}{|llll|}
    \hline
    Library: &
    Arity $0$: & \gena, \genb, \genc, \gend, \inputa, \inputb &
    \\ & 
    Arity $1$: &  \identity&
    \\ &
    Arity $2$: & \plus, \minus, \timess&
     \\ \hline
     Sketch: &
     block $\mathtt{C}$: & \gena, \genb, \genc, \gend, \plus(-,-), \minus(-,-), \timess(-,-) & 5 lines
     \\ &
  ...
  \caption{{\small{Inputs to the synthesis engine consists of the
        ...
        Charlie executes block $\mathtt{C}$. 
        Blocks $\mathtt{CA,CB}$ represent values sent by Charlie to
...
Let Alice's (private input) value be $\mathtt{input}(A)\in\mathbb{Z}_{32}$,
and Bob's value be $\mathtt{input}(B)\in\mathbb{Z}_{32}$.
For {\em correctness}, 
...
but does cannot influence its outcome.
...
follows  the formulation of the component-based synthesis problem~\cite{bitvector}.
...
\begin{figure*}
    \begin{eqnarray*}
      \ttrule{\gena}{}{\hoare{\theta}{v := \gena}{\theta \circ \{v \mapsto pol2vec(``x'')\}}}{ v_x = 1 \;\wedge\; \bigwedge_{i\neq x} v_i = 0}
        \\
        \ttrule{\genb}{}{\hoare{\theta}{v := \genb}{\theta \circ \{v \mapsto pol2vec(``y'')\}}}{ v_y = 1\;\wedge\; \bigwedge_{i\neq y} v_i = 0}
        \\
        \ttrule{\plus}{[\theta(y)\neq y, \theta(z)\neq z]}%
        ...
        \\
        \ttrule{\mathtt{check}}%
        {\hoare{\mathtt{id}}{P}{\theta} \qquad \theta(e_1) = \theta(e_2)}%
...
Rather than searching for the program, let us say we want to
search for the proof in the proof system shown in Figure~\ref{fig:rules}.  
Note that the
...
then define
$\mathit{isrand}(p)$  to be the Boolean value
$b_1\vee b_2\vee b_3\vee b_4$.

The logic to capture (b)  is long, but simple. 
For example, if Alice obtains a set $S = \{v_1, v_2, \ldots\}$ of values
from Bob and Charlie, then we check if, for some subset $S_1\subset S$ of $S$,
it is the case that
\begin{eqnarray*}
...
We now associate this program with three different dual components.
In all cases, $l_0,l_1,l_2,l_3$ are the four dual program variables.
The first dual program propagates information pertaining
to whether a value (computed at line $l_i$ in the primal) is a
constant.
The dual domain here $\domd$ is $\{\true,\false\}$, where, intuitively speaking,
if the dual variable $l_i$
is assigned $\true$, then the value computed on line $l_i$ in the primal
is known to be a ``constant'' value, whereas if it is $\false$,
...
From the conjunction of the four constraints, it is evident that we can not have $l_3$
...
\begin{remark}
  ...
  This is because the dual semantics should typically be designed to minimally
  constrain the dual variables. 
...
is shown in Column titled ``Dual 2''. In this case, the dual domain
...
Having the value $\zero$ allows us to generate constraints (in the dual program) about preservation of
...
\begin{remark}
  ...
  allow us to perform standard program analysis. In these cases, the dual meaning
  is an ``abstraction'' of the concrete primal semantics. % We will revisit this point
  % later. 
...
For a program path $\pi$, let us assume that we have
a partition of the primal program variables $\V$
used in $\pi$ into two sets: $\UndefV(\pi)$ and $\DefV(\pi)$.
Intuitively, $\DefV(\pi)$ contains variables that are
defined in $\pi$ and $\UndefV(\pi)$ contains all the other
variables (treated as inputs to $\pi$).

\begin{definition}
The dual component of a program $P$
is an {\underline{abstraction}} of its primal component,
in short {\underline{$\semd$ is an abstraction of $\semp$}},
...
for every complete path $\pi\in\paths(v_s,v_e)$ in the program, 
for every $\sigma\in\semp(\pi)(\psp)$, and
for every $\theta\in\semd(\pi)(\psd)$,
it is the case that
if $\sigma|_{\UndefV(\pi)}\in\gamma(\theta)|_{\UndefV(\pi)}$, then
...
\begin{example}
  ...
  under the
  natural concretization function, where
...
Duality has an interesting use in program verification: it replaces a 
