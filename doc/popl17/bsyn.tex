...
\title{Search for Proof to Find the Program: Duality for Bounded Program Synthesis}
...
Our tool, along with all the examples 
%presented in the following sections,
and corresponding 
SMT instances, is available at~\texttt{ommited for double-blindness}.
...
restriction that $C$ cannot use multiplication. This prevents $C$ %this
from generating appropriately correlated random data,
...
and hence using general sketches spanning a large ${\cal F}$
is fine as long as ${\cal F}$ contains a solution.
One the other hand, if the synthesis problem 
is not realizable, proving so for large families of programs may not scale well.
...
problem involves 3 cryptographers who decide to
...
Due to technical difficulties in formalizing the security requirements,
we used approximate requirements that eliminated a large number 
insecure protocols, but not necessarily all of them. %sufficient to 
Approches based on $\exists\forall$ paradigm failed due to the complexity
of the protocol.
However, we were able to perform synthesis 
...
%
\ignore{
The dual domain $\domd$ % of our dual interpretation 
is just a fixed length
...
%
Using the $\exists\exists$ approach, we synthesized two different OT protocols:
...
The approach in~\cite{TGD15:CADE} falls in the $\Psi_{\exists\forall}$
paradigm. Here, we introduce duality and the 
$\Phi_{\exists\exists}$ paradigm for synthesis, which speeds up synthesis
...
The dual interpretations behave as constraints, and hence
our combination of primal and dual has flavors of contraint
programming~\cite{constraintProgramming} and combining
inductive and co-inductive constructs~\cite{AbelCoPatterns}.
